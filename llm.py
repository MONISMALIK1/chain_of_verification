...
to disk. Backend-agnostic — OpenRouter by default, or point ``COVE_BASE_URL`` at a
...
CoVe leans on the model as its own fact-checker, so the verification calls want
stable, low-variance answers — ``chat`` defaults to greedy (temperature 0).
"""
...
DEFAULT_MODEL = os.environ.get("COVE_MODEL", "openai/gpt-oss-120b:free")
_API_KEY = os.environ.get("OPENROUTER_API_KEY") or os.environ.get("COVE_API_KEY", "")
_API_URL = os.environ.get("COVE_BASE_URL", "https://openrouter.ai/api/v1/chat/completions")
_MAX_RETRIES = 5
_RETRY_BASE = 2.0
...
def chat(
    ...
    if not _API_KEY and not _is_local(_API_URL):
        raise EnvironmentError(
            ...
            "For a local model: export COVE_BASE_URL=http://localhost:11434/v1/chat/completions "
    ...
    if "openrouter" in _API_URL:
        headers["HTTP-Referer"] = "https://github.com/MONISMALIK1/chain_of_verification"
        headers["X-Title"] = "Chain-of-Verification: self-fact-checking to reduce hallucination"
